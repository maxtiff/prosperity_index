"""
Test code to figure out data structure
"""
##2009-2013
fname = 'net_gross_us_13.txt'

# df_net_13=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','net_migration'],nrows=435121,converters={'fips':str})
df_net_13=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','2013'],converters={'fips':str})
# Get levels of factors and work through table to sum totals in order to get annual figure of net migration from B to A.
# df['fips']=df['fips'].astype('category')
df_net_13 = df_net_13[~df_net_13.fips.str.contains('^072|^\w{3}000')]
df_net_13['2013'].fillna(value=0,inplace=True)
df_net_13 = df_net_13.groupby(by=['fips'])['2013'].sum().to_frame().reset_index()

##2008-2012
fname = 'net_gross_us_12.txt'

# df_net_12=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','net_migration'],nrows=433949,converters={'fips':str})
df_net_12=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','2012'],converters={'fips':str})
# Get levels of factors and work through table to sum totals in order to get annual figure of net migration from B to A.
# df['fips']=df['fips'].astype('category')
df_net_12 = df_net_12[~df_net_12.fips.str.contains('^072|^\w{3}000')]
df_net_12['2012'].fillna(value=0,inplace=True)
df_net_12 = df_net_12.groupby(by=['fips'])['2012'].sum().to_frame().reset_index()

##2007-2011
fname = 'net_gross_us_11.txt'

# df_net_11=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','net_migration'],nrows=430882,converters={'fips':str})
df_net_11=pd.read_fwf(fname,colspecs=[(0,6),(180,190)],header=None,names=['fips','2011'],converters={'fips':str})
# Get levels of factors and work through table to sum totals in order to get annual figure of net migration from B to A.
# df['fips']=df['fips'].astype('category')
df_net_11 = df_net_11[~df_net_11.fips.str.contains('^072|^\w{3}000')]
df_net_11['2011'].fillna(value=0,inplace=True)
df_net_11 = df_net_11.groupby(by=['fips'])['2011'].sum().to_frame().reset_index()
	
##2006-2010
fname = 'ctyxcty_10.txt'
# raw_flow_10=pd.read_fwf(fname,colspecs=[(0,6),(6,12),(374,380)],header=None, names=['fips_current','fips_previous','movers'],nrows=251213,converters={'fips_current':str,'fips_previous':str}).fillna(value=0)
raw_flow_10=pd.read_fwf(fname,colspecs=[(0,6),(6,12),(374,380)],header=None, names=['fips_current','fips_previous','movers'],converters={'fips_current':str,'fips_previous':str}).fillna(value=0)
raw_flow_10 = raw_flow_10[~raw_flow_10.fips_current.str.contains('^072|^\w{3}000')]

raw_flow_10['fips_current']=raw_flow_10['fips_current'].astype('category')
raw_flow_10['fips_previous']=raw_flow_10['fips_previous'].astype('category')

current = raw_flow_10.groupby('fips_current')['movers'].sum()
previous = raw_flow_10.groupby('fips_previous')['movers'].sum().ix[:-10]

df_flow_10 = pd.concat([current,previous],axis=1,keys=['movers_in','movers_out'])
df_flow_10.fillna(value=0,inplace=True)
df_flow_10['2010'] = df_flow_10['movers_in']-df_flow_10['movers_out']
df_flow_10.drop(['movers_in','movers_out'],axis=1,inplace=True)
df_flow_10=df_flow_10.reset_index().rename(columns={'index':'fips'})

##2005-2009
fname = 'ctyxcty_09.txt'
# raw_flow_09=pd.read_fwf(fname,colspecs=[(0,6),(6,12),(374,380)],header=None, names=['fips_current','fips_previous','movers'],nrows=248030,converters={'fips_current':str,'fips_previous':str}).fillna(value=0)
raw_flow_09=pd.read_fwf(fname,colspecs=[(0,6),(6,12),(374,380)],header=None, names=['fips_current','fips_previous','movers'],converters={'fips_current':str,'fips_previous':str}).fillna(value=0)
raw_flow_09 = raw_flow_09[~raw_flow_09.fips_current.str.contains('^072|^\w{3}000')]

raw_flow_09['fips_current']=raw_flow_09['fips_current'].astype('category')
raw_flow_09['fips_previous']=raw_flow_09['fips_previous'].astype('category')

current = raw_flow_09.groupby('fips_current')['movers'].sum()
previous = raw_flow_09.groupby('fips_previous')['movers'].sum().ix[:-10]

df_flow_09 = pd.concat([current,previous],axis=1,keys=['movers_in','movers_out'])
df_flow_09.fillna(value=0,inplace=True)
df_flow_09['2009'] = df_flow_09['movers_in']-df_flow_09['movers_out']
df_flow_09.drop(['movers_in','movers_out'],axis=1,inplace=True)
df_flow_09=df_flow_09.reset_index().rename(columns={'index':'fips'})


#Bringing it all together
# crude_final = df_flow_09.merge(df_flow_10,on='fips').merge(df_net_11,on='fips').merge(df_net_12,on='fips').merge(df_net_13,on='fips')

def multiple_merge(lst_dfs, on):
    reduce_func = lambda left,right: pd.merge(left, right, on=on)

    return ft.reduce(reduce_func, lst_dfs)

dfs = [df_flow_09,df_flow_10,df_net_11,df_net_12,df_net_13]

test = multiple_merge(dfs,'fips')
test = pd.melt(test, id_vars='fips',var_name='Date').sort_values(['fips','Date'])

levels = test['fips'].unique()

import os, sys, pandas as pd, functools as ft

fname = 'tags.csv'

test = pd.read_csv(fname)


# !/usr/bin/python

############################################################################
#
# This Python script retrieves the XML polling feeds from Huffington Post
# The feeds are a structured representation of all of the polling data
# publicly accessible through their website. After retreiving the feeds, it
# produces an output file which contains summary statistics for the polling
# in each state (and D.C.), as of each date since May 22. The summary
# statistics are described below under "Output Format".
#
# In other words, the first 51 lines of output are summary statistics (s.s.)
# using all currently available polls (also known as "polls which ended
# before today"). The next 51 lines of output are the same s.s. recomputed,
# but omitting any polls which ended before yesterday. Then, the 51 lines
# of s.s., omitting the polls which ended before two days ago, and so on,
# so that we have the history of these s.s. and can observe the effect of
# new polls.
#
# Author: Andrew Ferguson <adferguson@alumni.princeton.edu>
#
# Script written for election.princeton.edu run by Samuel S.-H. Wang under
# noncommercial-use-only license:
# You may use or modify this software, but only for noncommericial purposes.
# To seek a commercial-use license, contact sswang@princeton.edu
#
# Update History:
#    Oct 28, 2016 -- Add abbreviations for individual ME, NE districts
#    Jul  6, 2016 -- Temporary manual reset of states to reflect lone polls
#    Aug 19, 2012 -- moved to GitHub; future updates in commit messages
#    Jul 10, 2012 -- Updated for new HuffPo XML format
#    Jul  8, 2012 -- Export CSV files for exploratory analysis
#    Jul  7, 2012 -- Initial port from 2008 version
#
############################################################################

import os, sys, urllib2, datetime, time, xml.dom.minidom, socket
from numpy import *

############################################################################
#
# Global configuration and variables
#
############################################################################

output_filename = "2016.EV.polls.median.txt"
midtype = "median"
num_recent_polls_to_use = 3
huffpo_base_url = ""
huffpo_childNodes_per_page = 21  # 10 polls per page, plus space childNode on either side
archive_dir = "archive/ev/"

huffpo_base_url = 'http://elections.huffingtonpost.com/pollster/api/polls.xml?question=16-%s-Pres-GE%%20TrumpvClinton&page=%s'

state_polls = {}
# state_polls is a dictionary with each an entry for each state. Each entry
# is a list of tuples. Each tuple represents one poll and is of the form:
# (margin, start date, end date, mid date, population, polling organization)

prev_outcome = {}
# prev_outcome is a dictionary like state_polls with a single tuple recording
# the outcome from the previous election for states which have had sparse
# polling, if any, during the current campaign

poll_ids = []
max_filenum = 0

# Files for exploratory analysis
state_filename = "2016_StatePolls.csv"
state_file = None
national_filename = "2016_NationalPolls.csv"
national_file = None

us_state_abbrev = {
    'alabama': 'AL',
    'alaska': 'AK',
    'arizona': 'AZ',
    'arkansas': 'AR',
    'california': 'CA',
    'colorado': 'CO',
    'connecticut': 'CT',
    'delaware': 'DE',
    'florida': 'FL',
    'georgia': 'GA',
    'hawaii': 'HI',
    'idaho': 'ID',
    'illinois': 'IL',
    'indiana': 'IN',
    'iowa': 'IA',
    'kansas': 'KS',
    'kentucky': 'KY',
    'louisiana': 'LA',
    'maine': 'ME',
    'maine': 'M1',
    'maine': 'M2',
    'maryland': 'MD',
    'massachusetts': 'MA',
    'michigan': 'MI',
    'minnesota': 'MN',
    'mississippi': 'MS',
    'missouri': 'MO',
    'montana': 'MT',
    'nebraska': 'NE',
    'nebraska': 'N1',
    'nebraska': 'N2',
    'nebraska': 'N3',
    'nevada': 'NV',
    'new-hampshire': 'NH',
    'new-jersey': 'NJ',
    'new-mexico': 'NM',
    'new-york': 'NY',
    'north-carolina': 'NC',
    'north-dakota': 'ND',
    'ohio': 'OH',
    'oklahoma': 'OK',
    'oregon': 'OR',
    'pennsylvania': 'PA',
    'rhode-island': 'RI',
    'south-carolina': 'SC',
    'south-dakota': 'SD',
    'tennessee': 'TN',
    'texas': 'TX',
    'utah': 'UT',
    'vermont': 'VT',
    'virginia': 'VA',
    'washington': 'WA',
    'west-virginia': 'WV',
    'wisconsin': 'WI',
    'wyoming': 'WY',
    'united-states': 'US',  # not sure if this is right
}


############################################################################
#
# Main
#
############################################################################

def main():
    global max_filenum
    global midtype
    global output_filename
    global huffpo_base_url
    global state_file, national_file

    try:
        if sys.argv[1] == "--mean":
            midtype = "mean"
            output_filename = "polls.mean.txt"
    except:
        pass

    for state in state_names:
        state_polls[state] = []

    state_file = init_analysis_file(state_filename)
    national_file = init_analysis_file(national_filename)
    store_prev_outcome()

    # load base url
    # huffpo_config = open(".huffpo.url")
    # huffpo_base_url = huffpo_config.readline()[:-1]
    # huffpo_config.close()

    # process archive of polls XXX why were we doing this? We get all polls anyway...

    # for fname in os.listdir(archive_dir):
    #    n = int(fname.split(".")[0])
    #    if n > max_filenum:
    #        max_filenum = n
    #    parse_pollfile(archive_dir + fname)

    # get the latest polls
    socket.setdefaulttimeout(5)
    fetch_latest_polls()

    process_polls()

    state_file.close()
    national_file.close()


def init_analysis_file(filename):
    outfile = open(filename, "w+")

    outfile.write("State,pollster,pop,vtype,method,begmm,begdd,begyy,")
    outfile.write("endmm,enddd,endyy,trump,clinton,other,undecided,")
    outfile.write("Begdate,Enddate,Middate\n")

    return outfile


############################################################################
#
# Returns the median and std. error
#
############################################################################

# The std. error is given by:
#         std. deviation / sqrt(num of polls)
#
# We robustly estimate the std. deviation from the median absolute
# deviation (MAD) using the standard formula:
#        std. deviation =  MAD / invcdf(0.75)
#
# The MAD is defined as median( abs[samples - median(samples)] )
# invcdf(0.75) is approximately 0.6745

def get_statistics(set):
    [margins, sdates, edates, mdates, pop, poll_orgs] = zip(*set)
    set = array(margins)

    num = set.size
    assert num >= 3

    if midtype == "median":
        median_margin = median(set)
        mad = median(abs(set - median_margin))
        sem_est = mad / 0.6745 / sqrt(num)

        return (median_margin, sem_est)

    else:
        assert midtype == "mean"

        mean_margin = mean(set)
        sem = std(set) / sqrt(num)

        return (mean_margin, sem)


# Special case for when only two polls are available

def get_two_statistics(set):
    [margins, sdates, edates, mdates, pop, poll_args] = zip(*set)
    set = array(margins)

    assert set.size == 2

    mean_margin = mean(set)
    sem = max(std(set) / sqrt(set.size), 3)

    return (mean_margin, sem)


############################################################################
#
# Functions to write the statistics which Sam's MATLAB scripts will use as
# inputs. There is one line of statistics for each state and D.C., for
# each date from today back to May 22
#
############################################################################

# Output Format -- 5 numbers per state
#    - number of polls used for average
#    - end date of oldest poll used (January 1= 1)
#    - average margin where margin>0 is Clinton ahead of Trump
#    - estimated SEM of margin
#    - analysisdate (written by the 'process_polls' method)

# Remember, the format of the tuple for each list:
# (margin, start date, end date, mid date, pop, polling organization)

def write_statistics(pfile, polls, nopolls):
    # Number of polls available on this date for this state
    num = len(polls)

    if num == 0:
        assert False
    elif num == 1:
        pfile.write("%s " % (
        0 if nopolls else num))  # if there were no real polls, write 0 in first column
        pfile.write("%s " % int(polls[0][2].strftime("%j")))
        pfile.write("%s " % polls[0][0])
        pfile.write("%s " % sqrt(1.0 / polls[0][4]))
    elif num == 2:
        pfile.write("%s " % num)
        pfile.write("%s " % int(polls[1][2].strftime("%j")))
        pfile.write("%s %s " % get_two_statistics(polls))
    else:
        # We want to use only the three most recent polls, as defined by the
        # end date, and allowing for ties
        polls.sort(key=lambda x: x[2], reverse=True)
        third_date = polls[num_recent_polls_to_use - 1][2]
        last_three = filter(lambda x: x[2] >= third_date, polls)

        # Or, if this gives more polls, use the polls from the 7 days prior
        # to the most recent poll.
        polls.sort(key=lambda x: x[2], reverse=True)
        most_recent_date = polls[0][2]
        seven_days_prior = most_recent_date - datetime.timedelta(7, 0, 0)
        last_seven_days = filter(lambda x: x[2] >= seven_days_prior, polls)

        if (len(last_seven_days) > len(last_three)):
            working_subset = last_seven_days
        else:
            working_subset = last_three

        pfile.write("%s " % len(working_subset))
        pfile.write("%s " % int(working_subset[-1][2].strftime("%j")))
        pfile.write("%s %s " % get_statistics(working_subset))


def drop_overlapping_polls(polls, state):
    if len(polls) <= 1:
        return polls

    cleaned_polls = []

    # Reverse the order [::-1] so when multiple entries have the same pollster and date,
    # the poll accepted is the first one that was originally entered into the feed.
    # The motivation here is to respect each pollster's judgment about the topline result.
    # sort by: pollster, start date (old to new), end date (old to new)
    by_pollster = sorted(polls[::-1], key=lambda x: (x[5], x[1], x[2]))

    prev_pollster = by_pollster[0][5]
    prev_start = by_pollster[0][1]
    prev_end = by_pollster[0][2]
    prev_poll = by_pollster[0]
    cleaned_polls.append(by_pollster[0])

    for poll in by_pollster[1:]:
        if prev_pollster != poll[5]:
            prev_pollster = poll[5]
        else:
            #    if prev_end > poll[1]:
            cleaned_polls.pop()

        prev_start = poll[1]
        prev_end = poll[2]
        prev_poll = poll
        cleaned_polls.append(poll)

    # write_statistics expects polls to be sorted from newest to oldest
    # by mid-date
    cleaned_polls.sort(key=lambda x: x[3], reverse=True)
    return cleaned_polls


def process_polls():
    pfile = open(output_filename, "w")

    # Make sure the polls are sorted with most recent first
    for state in state_polls.keys():
        polls = state_polls[state]
        polls.sort(key=lambda x: x[2], reverse=True)

    # Write the 51 lines of output statistics for each day, from now
    # back to May 22
    for day in campaign_season():
        statenum = 1
        for state in sorted(state_names, key=lambda x: state_names[x]):
            polls = state_polls[state]

            # Remember, the format of the tuple for each list:
            # (margin, start date, end date, mid date, pop, polling org)
            polls_ended_before_day = filter(lambda x: x[2] <= day, polls)

            #  reject two polls by the same pollster if their
            # [startdate,enddate] intervals overlap (i.e. if older
            # enddate>=newer startdate).
            cleaned_polls = drop_overlapping_polls(polls_ended_before_day,
                                                   state)

            nopolls = False
            if len(cleaned_polls) == 0:
                cleaned_polls = [prev_outcome[state]]
                nopolls = True

            write_statistics(pfile, cleaned_polls, nopolls)
            pfile.write("%s %d\n" % (int(day.strftime("%j")), statenum))
            statenum += 1

    pfile.close()


############################################################################
#
# Functions to fetch the latest polls from the Huffington Post
#
############################################################################


def url_fetcher(page_num, state):
    tries = 0
    while True:
        try:
            f = urllib2.urlopen(huffpo_base_url % (state, str(page_num)))
            return f
        except urllib2.URLError:
            if tries < 3:
                tries += 1
                time.sleep(2)
            else:
                print
                "FAIL on page %d (tried three times)" % page_num
                sys.stdout.flush()
                raise


def fetch_latest_polls():
    global max_filenum

    for state in us_state_abbrev.values():
        print
        "STATE", state
        page_num = 0
        stop = False
        while (not stop):
            page_num += 1
            print
            "Fetching page %s for %s" % (str(page_num), state)
            page = url_fetcher(page_num, state)

            (xmldoc, stop) = parse_pollfile(page, state)
            xmldoc.childNodes[0].normalize()

            if len(xmldoc.childNodes[0].childNodes) > 1:
                max_filenum += 1
                f = open("%s%s%s.xml" % (archive_dir, state, str(page_num)),
                         'w')
                s = xmldoc.toxml(encoding="utf-8")
                f.write(s)
                f.close()
                print
                "Wrote %d polls to %s" % (
                0.5 * (len(xmldoc.childNodes[0].childNodes) - 1),
                "%s%s%s.xml" % (archive_dir, state, str(page_num)))

            if not stop:
                time.sleep(1)


############################################################################
#
# Functions to process poll data from Huffington Post. Adds tuples of
# the form (margin, start_date, end_date, mid_date, pop, poll_org) to the
# state_polls dictionary. The margin is defined as: Dem - GOP
#
############################################################################


def parse_pollfile(filename, state):
    (xmldoc, stop) = unique_polls(filename)
    process_pollfile(xmldoc, state)
    return (xmldoc, stop)


def unique_polls(filename):
    xmldoc = xml.dom.minidom.parse(filename)

    pi_nodes = xmldoc.getElementsByTagName("id")

    if len(pi_nodes) == 0:
        return (xmldoc, True)

    for pi_node in pi_nodes:
        poll_id = int(pi_node.childNodes[0].nodeValue)

        if poll_id in poll_ids:
            poll_node = pi_node.parentNode
            polls = poll_node.parentNode
            polls.removeChild(poll_node)
        else:
            poll_ids.append(poll_id)

    return (xmldoc, False)


def get_opt_subelem(elem, name, default):
    tmp = elem.getElementsByTagName(name)
    if len(tmp) > 0 and tmp[0].hasChildNodes():
        return tmp[0].childNodes[0].nodeValue
    return default


def subpop_parse(subpop):
    values = {"Clinton": "", "Trump": "", "Other": "", "Undecided": ""}

    vtype = get_opt_subelem(subpop, "name", "")
    pop = int(get_opt_subelem(subpop, "observations",
                              "1"))  # if no population size, pretend it's 1

    responses = subpop.getElementsByTagName("response")

    for r in responses:
        candidate = r.getElementsByTagName("choice")[0].childNodes[0].nodeValue
        value = r.getElementsByTagName("value")[0].childNodes[0].nodeValue
        for v in values.keys():
            if candidate.count(v):
                candidate = v
        values[candidate] = value

    assert (values["Clinton"] != "")
    assert (values["Trump"] != "")

    return (vtype, pop, values)


def process_subpop(poll_org, method, state, start_date, end_date, subpop,
                   question_state):
    global state_file, national_file

    (vtype, pop, values) = subpop_parse(subpop)

    mid_date = start_date + ((end_date - start_date) / 2)
    margin = float(values["Clinton"]) - float(values["Trump"])
    f = None

    if state != "US":
        state_polls[question_state].append((margin, start_date, end_date,
                                            mid_date, pop, poll_org))
        # state_polls[state].append((margin, start_date, end_date,
        #                           mid_date, pop, poll_org))
        f = state_file
    else:
        f = national_file

    d = (start_date.month, start_date.day, start_date.year,
         end_date.month, end_date.day, end_date.year,
         mid_date.month, mid_date.day, mid_date.year)
    f.write("%s,\"%s\",%d,\"%s\",\"%s\"," % (
    question_state, poll_org, pop, vtype, method))
    f.write("%d,%d,%d,%d,%d,%d," % d[:6])
    f.write("%s,%s,%s,%s," % (values["Trump"], values["Clinton"],
                              values["Other"], values["Undecided"]))
    f.write("%s/%s/%s,%s/%s/%s,%s/%s/%s\n" % d)


def process_pollfile(xmldoc, state):
    codes = xmldoc.getElementsByTagName("code")
    presCodes = filter(
        lambda x: x.hasChildNodes() and x.childNodes[0].nodeValue.endswith(
            "Pres-GE TrumpvClinton"), codes)
    questions = map(lambda x: x.parentNode, presCodes)

    for q in questions:
        try:
            poll = q.parentNode.parentNode
            poll_org = poll.getElementsByTagName("pollster")[0].childNodes[
                0].nodeValue
            method = get_opt_subelem(poll, "method", "")
            start_date = strpdate(
                poll.getElementsByTagName("start_date")[0].childNodes[
                    0].nodeValue)
            end_date = strpdate(
                poll.getElementsByTagName("end_date")[0].childNodes[
                    0].nodeValue)
            poll_state = q.getElementsByTagName("state")[0].childNodes[
                0].nodeValue

            # if state == 'CO' and poll_org == 'Ipsos/Reuters':
            #	print q.getElementsByTagName("state")[0].childNodes[0].nodeValue

            if start_date < datetime.date(2016, 1, 1):
                continue

            subpops = q.getElementsByTagName("subpopulation")

            if len(subpops) >= 2:
                for subpop in subpops:
                    vtype = get_opt_subelem(subpop, "name", "")
                    if vtype == "Likely Voter":
                        process_subpop(poll_org, method, state, start_date,
                                       end_date, subpop, poll_state)
            else:
                for subpop in subpops:
                    process_subpop(poll_org, method, state, start_date,
                                   end_date, subpop, poll_state)
        except Exception as e:
            try:
                print
                "Problem processing poll #%s" % (
                q.parentNode.parentNode.getElementsByTagName("id")[
                    0].childNodes[0].nodeValue)
                print
                e
                # print q.parentNode.parentNode.toprettyxml(encoding="utf-8")
            except:
                print
                "Very bad problem with below question. Ignoring:"
                # print q.toprettyxml(encoding="utf-8")


############################################################################
#
# Utility functions and data
#
############################################################################

# Parses a date_string into a datetime.date. Included here to support
# Python 2.4, which lacks datetime.datetime.strpdate()

def strpdate(date_string, format="%Y-%m-%d"):
    return datetime.date(*(time.strptime(date_string, format)[0:3]))


# Generates all of the dates in the general election campaign season,
# starting from today and working back to May 22nd

def campaign_season():
    day = datetime.date.today()
    # start = datetime.date(2012, 5, 22)
    start = datetime.date(2016, 5, 22)

    while day >= start:
        yield day
        day = day - datetime.timedelta(1, 0, 0)


# Hard-code 2012 results for several states since there is no current
# polling data and political analysts do not disagree on the expected outcome.
# temporarily change AR from -24 to -11 to patch over problem in feed - July 6 2016 SW
# temporarily change CO from 5 to 1 to patch over problem in feed
# temporarily change CT from 17 to 7.5 to patch over problem in feed
# temporarily change IL from 16 to 25 to patch over problem in feed
# temporarily change IN from -7.5 to -8 to patch over problem in feed
# temporarily change KS from -22 to -1.5 to patch over problem in feed
# temporarily change LA from -17 to -16 to patch over problem in feed
# temporarily change MA from 23 to 30 to patch over problem in feed
# temporarily change ME from 15 to 7 to patch over problem in feed
# temporarily change NV from 7 to -7 to patch over problem in feed
# temporarily change TN from -20 to -9 to patch over problem in feed
# temporarily change TX from -16 to -8 to patch over problem in feed
# temporarily change WA from 15 to 12 to patch over problem in feed

def store_prev_outcome():
    prev_outcome["AL"] = (-22, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["AK"] = (-14, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["AR"] = (-11, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["CO"] = (1, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["CT"] = (7.5, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["DC"] = (83, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["DE"] = (19, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["HI"] = (43, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["ID"] = (-32, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["IL"] = (25, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["IN"] = (-8, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["KS"] = (-1.5, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["KY"] = (-23, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["LA"] = (-16, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["ME"] = (7, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["MA"] = (30, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["MS"] = (-3, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["MT"] = (-14, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["NE"] = (-22, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["NV"] = (7, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["NJ"] = (11, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["NM"] = (8, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["ND"] = (-20, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["OK"] = (-34, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["OR"] = (4.5, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["RI"] = (28, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["SC"] = (-10, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["SD"] = (-18, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["TN"] = (-9, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["TX"] = (-8, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["UT"] = (-5.5, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["VT"] = (36, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["WA"] = (12, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["WV"] = (-27, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    prev_outcome["WY"] = (-41, datetime.date(2016, 1, 1),
                          datetime.date(2016, 1, 1), datetime.date(2016, 1, 1),
                          1, "Election 2012")
    # put dummy data for every state. Remove loop once real data from 2012 is acquired
    # for state in state_names:
    #     prev_outcome[state] = (0, datetime.date(1900,1,1), datetime.date(1900,1,1), datetime.date(1900,1,1), 1, "BAD DATA")


state_names = {
    'AL': 'Alabama',
    'AK': 'Alaska',
    'AZ': 'Arizona',
    'AR': 'Arkansas',
    'CA': 'California',
    'CO': 'Colorado',
    'CT': 'Connecticut',
    'DC': 'D.C.',
    'DE': 'Delaware',
    'FL': 'Florida',
    'GA': 'Georgia',
    'HI': 'Hawaii',
    'ID': 'Idaho',
    'IL': 'Illinois',
    'IN': 'Indiana',
    'IA': 'Iowa',
    'KS': 'Kansas',
    'KY': 'Kentucky',
    'LA': 'Louisiana',
    'ME': 'Maine',
    'MD': 'Maryland',
    'MA': 'Massachusetts',
    'MI': 'Michigan',
    'MN': 'Minnesota',
    'MS': 'Mississippi',
    'MO': 'Missouri',
    'MT': 'Montana',
    'NE': 'Nebraska',
    'NV': 'Nevada',
    'NH': 'New Hampshire',
    'NJ': 'New Jersey',
    'NM': 'New Mexico',
    'NY': 'New York',
    'NC': 'North Carolina',
    'ND': 'North Dakota',
    'OH': 'Ohio',
    'OK': 'Oklahoma',
    'OR': 'Oregon',
    'PA': 'Pennsylvania',
    'RI': 'Rhode Island',
    'SC': 'South Carolina',
    'SD': 'South Dakota',
    'TN': 'Tennessee',
    'TX': 'Texas',
    'UT': 'Utah',
    'VT': 'Vermont',
    'VA': 'Virginia',
    'WA': 'Washington',
    'WV': 'West Virginia',
    'WI': 'Wisconsin',
    'WY': 'Wyoming'
}

if __name__ == '__main__':
    try:
        main()
    except KeyboardInterrupt:
        pass